...
function soloNumeros(array) {
  ...
  // Tu código aca:
  let enteros =  array.filter(function(numEnteros) {
    return typeof numEnteros === 'number';
  })
  return enteros;
...
function sumaTodos(array) {
  ...
  let sumTodos = [];
  let suma = 0;
  const sumaRango = (accumulator, currentValue) => accumulator + currentValue;
  ...
  suma = sumTodos.reduce(sumaRango);
  return suma; 
...
var inventario = [
  ...
  let busqueda = inventario.find(elemento => elemento.nombre === item);
  let totalBuscar = busqueda.cantidad;
  if(totalBuscar >= 0){
    return totalBuscar;
  }else{
    return 0;
...
function numeroSimetrico(num) {
  ...
  let resultado = Number(numReverse);
  if(resultado === num){
  return true;
    }else{
  return false;
...
// Tu código aca:
    Array.prototype.encontrarIndex = function (valor, desde = 0) {
...
function crearClasePersona(nombre,edad,hobbies,amigos) {
